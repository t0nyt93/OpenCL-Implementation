...
main( int argc, char *argv[ ] )
{
	...
	if( status != CL_SUCCESS )
		...
		char *str;
		clGetPlatformInfo(platform[1], CL_PLATFORM_NAME, 0, NULL, &size);
		str = new char[size];
		clGetPlatformInfo(platform[1], CL_PLATFORM_NAME, size, str, NULL);
		printf( "\tName = '%s'\n", str);
		delete[] str;
	// get the device id:
	...
	clGetDeviceInfo(device, CL_DEVICE_VENDOR_ID, sizeof(ui), &ui, NULL);
	printf( "\t\tDevice Vendor ID = 0x%04x\n", ui);
	clGetDeviceInfo(device, CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(ui), &ui, NULL);
	printf( "\t\tDevice Maximum Compute Units = %d\n", ui);
	clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(ui), &ui, NULL);
	...
	clGetDeviceInfo(device, CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(ui), &ui, NULL);
